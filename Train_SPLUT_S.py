...
I_VALIDATION = 2000  # validate every N iteration
...
class SRNet(paddle.nn.Layer):
    ...
    def forward(self, x_in):
        ...
        x_temp=x_in
        x221 = self.lut221(self.quant(F.pad(x[:,0:2,:,:],(0,0,0,1), mode='reflect')+F.pad(x[:,2: ,:,:],(0,0,1,0), mode='reflect')))
        x212 = self.lut212(self.quant(F.pad(x[:,0:2,:,:],(0,1,0,0), mode='reflect')+F.pad(x[:,2: ,:,:],(1,0,0,0), mode='reflect')))
        x=(x221+x212)/2.0+x

        x2 = self.lut221_c12(self.quant(F.pad(x[:,0:2,:,:],(0,0,0,1), mode='reflect')+F.pad(x[:,2: ,:,:],(0,0,1,0), mode='reflect')))
        x3 = self.lut212_c34(self.quant(F.pad(x[:,0:2,:,:],(0,1,0,0), mode='reflect')+F.pad(x[:,2: ,:,:],(1,0,0,0), mode='reflect')))
...
if __name__ == '__main__':
    ...
    for i in tqdm(range(START_ITER+1, NB_ITER+1)):
        ...
        batch_H = paddle.to_tensor(batch_H)      # BxCxHxW, range [0,1]
        batch_L = paddle.to_tensor(batch_L)     # BxCxHxW, range [0,1]

        #[0,1]-->[0,255]
        batch_L225 = paddle.floor(batch_L*255)
        #batch_L_A=(batch_L225// QQ)*QQ
        batch_L_A = paddle.floor_divide(paddle.to_tensor(batch_L225, dtype='int64'),
                                        paddle.to_tensor(QQ, dtype='int64'))
        batch_L_A*=QQ
        batch_L_B= batch_L225 % QQ
        batch_L_A=batch_L_A/255.0
        batch_L_B=batch_L_B/255.0
